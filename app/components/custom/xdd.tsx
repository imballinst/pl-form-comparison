...
export const makeData = (num, columns) =>
  [...Array(num)].map(() => ({
    ...Object.fromEntries(columns.map((col) => [col.accessorKey, 'hehe'])),
  }))

export type Person = ReturnType<typeof makeData>[0]

export function Xdd() {
  const columns = React.useMemo<ColumnDef<Person>[]>(() => makeColumns(24), [])
  ...
  return (
    <div className="app">
      {process.env.NODE_ENV === 'development' ? (
        <p>
          <strong>Notice:</strong> You are currently running React in development mode. Virtualized rendering performance will be slightly
...
function TableBody({ columnVirtualizer, table, tableContainerRef, virtualPaddingLeft, virtualPaddingRight }: TableBodyProps) {
